#!/usr/bin/env python3
"""
Script to fix recently queued jobs that weren't properly dispatched to Celery
"""
...
def fix_recent_queued_jobs():
    """Fix recent queued jobs that don't have Celery task IDs"""

    with SessionLocal() as db:
        try:
            print("🔄 Fixing recent queued jobs...")

            # Find all recent queued jobs without task IDs (last 24 hours)
            ...
            print(f"📊 Found {len(queued_jobs)} recent jobs to fix")

            if len(queued_jobs) == 0:
                print("✅ No recent jobs to fix")
                return
...
if __name__ == "__main__":
    print("🔧 Recent Queued Job Fixer")
    print(
        "This will re-dispatch Celery tasks for recent jobs that are stuck in 'queued' status"
    )
    print("===============================================================\n")

    fix_recent_queued_jobs()