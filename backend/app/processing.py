...
def upload_to_s3(bucket: str, key: str, data: bytes, content_type: str = "application/octet-stream"):
...
def rasterize_image(image_data: bytes, dpi: int = 300) -> List[Tuple[int, bytes]]:
    ...
    if hasattr(image, 'info') and 'dpi' in image.info:
        current_dpi = image.info['dpi'][0] if isinstance(image.info['dpi'], tuple) else image.info['dpi']
        if current_dpi != dpi:
...
def generate_tiles(image_data: bytes, tile_size: int = 256, quality: int = 80) -> List[Tuple[int, int, bytes]]:
...
def generate_thumbnail(image_data: bytes, max_size: Tuple[int, int] = (200, 300)) -> bytes:
...
def apply_redactions_to_image(image_data: bytes, redaction_regions: List[Tuple[int, int, int, int]]) -> bytes:
